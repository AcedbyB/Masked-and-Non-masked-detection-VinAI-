...
def drawBoxes(im, boxes):
    ...
    for i in range(x1.shape[0]):
        ...
        crop_img = im[int(y1[i]):int(y2[i]), int(x1[i]):int(x2[i])]
        if not crop_img is None:
            cv2.imwrite("/home/group-3/Downloads/cropped_10000/" + str(cnt) + ".jpg", crop_img)
            list_of_images.append(crop_img)
    return im
...
def main(imgfilepath):
    ...
    caffe_model_path = "/home/group-3/Desktop/mtcnn/model"
    threshold = [0.6, 0.7, 0.7]
    factor = 0.709
    ...
    RNet = caffe.Net(caffe_model_path+"/det2.prototxt", caffe_model_path+"/det2.caffemodel", caffe.TEST)
    ONet = caffe.Net(caffe_model_path+"/det3.prototxt", caffe_model_path+"/det3.caffemodel", caffe.TEST)
    for imgpath in glob.glob(imgfilepath + '*.jpg'):
